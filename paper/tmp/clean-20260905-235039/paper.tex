...
\section{Conclusions}
A matched RTX 4050 sweep localizes the CPU/WGPU reversal and quantifies static-policy regret. A separate ablation shows that eligibility preparation changes completed latency. The small-collection results add fast experimental WGPU and tensor paths, CPU wins on selective cells, resource costs, and a rejected selector. Android, Arc, and A6000 extend the observations within separate correctness and platform boundaries.

Routing should account for eligibility, representation, batching, runtime, and state. The evidence does not establish a universal threshold, a numerical-equivalence theorem, or a held-out advantage for an adaptive router. Final local selection needs new matched measurements before it inherits the corrected experimental latencies.

\section*{Reproducibility statement}
Retained evidence only: no product or benchmark changes and no experimental reruns. Ledgers preserve claims, revisions and hashes, environments, provenance, timing, counts, correctness, memory, qualification, and exclusions. Paper-only scripts regenerate reductions, tables, and PDF/PNG figures without replacing raw evidence or historical figures. Appendix~\ref{sec:artifact} and \path{paper/REPRODUCE.md} describe the audit and clean build.
