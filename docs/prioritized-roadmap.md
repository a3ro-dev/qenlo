...
| Publish the paper/writeup | The [research paper](../QENLO-RESEARCH-PAPER.pdf) (source `paper/v2/`) and the preserved [v1 evidence audit](../paper/archive/qenlo-evidence-audit-v1.pdf), with its source, figures, claim ledger and [reproduction instructions](../paper/REPRODUCE.md), are committed. The next experiments are specified in `research/specs/2026-09-25-clock-lock-and-faiss.md`. | Repository publication complete. A DOI, arXiv entry, or external engineering article remains optional distribution work. |
