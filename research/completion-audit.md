...
| Figures | Seven publication figures generated as vector PDF plus PNG: architecture, two crossover panels, static-policy regret, eligibility ablation, matched Windows strategies, matched Android panels, and descriptive Linux context | complete without pooled or invented curves |
...
| LaTeX paper | `paper/paper.tex`, `appendix.tex`, compiled `paper.pdf`, stable export under `paper/output/pdf/` | complete; all 13 rendered pages visually inspected, with no overflow or unresolved-reference warnings |
| Reproduction instructions | `paper/REPRODUCE.md`, campaign/analyze/plot scripts, immutable tag `paper-adaptive-routing-v1` | complete for headline cohorts; contextual caveats remain in the claim ledger |
| Raw and processed evidence | `research/data/raw`, `research/data/processed`, retained benchmark trees | complete |
| Archive-wide reanalysis | `research/scripts/analyze_full_archive.py`; `research/data/processed/archive-reanalysis/` | complete for tracked/selected sample series and four hash-verified synthesis archives; incompatible schemas/cohorts remain separate |
| Resource cleanup/budget | Runpod API: zero pods, total spend $0.9926765 | complete |

The requested full Cartesian experiment matrix is not complete and cannot be represented as such. Its absent cells are part of the scientific result: native WGPU was unavailable on the budgeted A6000 image, while rerunning all dimensions, distributions, batches, and HNSW expansions would exceed the bounded campaign and the available prepared datasets. The paper narrows every conclusion accordingly. The alpha.5 scalar policy was frozen and evaluated held out; it failed with 8/16 wrong routes and 235.7% maximum backend-choice regret. No successful multidimensional calibrated policy has been evaluated.
