...
async fn test_browser_session_lifecycle() {
    ...
    let stats = session.create_collection(&col_path, 3).await.expect("create collection");
    assert_eq!(stats.dimension, 3);
    assert_eq!(stats.rows, 0);

    // Add 3 records
    session.add_record(1, 10, 100, &[1.0, 0.0, 0.0]).expect("insert 1");
    session.add_record(2, 10, 200, &[0.0, 1.0, 0.0]).expect("insert 2");
    session.add_record(3, 20, 300, &[0.0, 0.0, 1.0]).expect("insert 3");
    ...
    let search_res = session.search(&[1.0, 0.0, 0.0], &Filter::ALL, 5).await.expect("search");
    ...
    let filtered_search = session.search(
        &[1.0, 0.0, 0.0],
        &Filter::new(Some(20), TimestampRange::ALL),
        5
    ).await.expect("filtered search");
    ...
    assert!(storage.files.iter().any(|f| f.name.ends_with(".qdb") || f.name == "HEAD"));
}
