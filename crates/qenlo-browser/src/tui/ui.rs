...
fn render_header(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let title_line = Line::from(vec![
        Span::styled(" ⬡ QENLO ", Style::default().fg(QENLO_THEME.bg).bg(QENLO_THEME.accent).add_modifier(Modifier::BOLD)),
        Span::styled(" BROWSER ", Style::default().fg(QENLO_THEME.accent).add_modifier(Modifier::BOLD)),
        Span::raw(" │ "),
        Span::styled(format!("📁 {path_str}"), Style::default().fg(QENLO_THEME.text).add_modifier(Modifier::BOLD)),
        Span::raw(" │ "),
        Span::styled(format!("Dim: {}D", app.status.dimension), Style::default().fg(QENLO_THEME.accent)),
        Span::raw(" │ "),
        Span::styled(format!("Live: {} ({})", app.status.live_rows, app.total_records), Style::default().fg(QENLO_THEME.text)),
        Span::raw(" │ "),
        Span::styled(format!("Gen: #{}", app.status.generation), Style::default().fg(QENLO_THEME.text_muted)),
        Span::raw(" │ "),
        Span::styled(if app.status.open { "● READY" } else { "○ CLOSED" }, Style::default().fg(status_color).add_modifier(Modifier::BOLD)),
    ]);
    ...
    let p = Paragraph::new(title_line).block(block).alignment(Alignment::Left);
...
fn render_rows_view(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let header_row = Row::new(vec![
        cell_styled(" ID", Style::default().fg(QENLO_THEME.accent).add_modifier(Modifier::BOLD)),
        cell_styled("USER ID", Style::default().fg(QENLO_THEME.text_muted).add_modifier(Modifier::BOLD)),
        cell_styled("TIMESTAMP", Style::default().fg(QENLO_THEME.text_muted).add_modifier(Modifier::BOLD)),
        cell_styled("STATUS", Style::default().fg(QENLO_THEME.text_muted).add_modifier(Modifier::BOLD)),
        cell_styled("VECTOR PREVIEW", Style::default().fg(QENLO_THEME.text_muted).add_modifier(Modifier::BOLD)),
    ])
    ...
    let rows: Vec<Row> = app
        ...
        .map(|(idx, r)| {
            ...
            let vec_preview: String = r
                ...
                .collect::<Vec<_>>()
                .join(", ");
            let vec_str = format!("[{vec_preview}{}]", if r.vector.len() > 4 { ", …" } else { "" });
            ...
            let mut row = Row::new(vec![
                ...
                Line::from(Span::styled(vec_str, Style::default().fg(QENLO_THEME.text_muted))),
            ]);
        ...
        .block(
            Block::default()
                ...
                .title(Span::styled(
                    ...
                    Style::default().fg(QENLO_THEME.text).add_modifier(Modifier::BOLD),
                ))
                .style(Style::default().bg(QENLO_THEME.surface)),
        );
    ...
    // Pagination info
    let page_start = if app.total_records == 0 { 0 } else { app.page_offset + 1 };
...
fn render_search_view(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let vec_block = Block::default()
        .borders(Borders::ALL)
        .border_style(if app.search_active_field == 0 { Style::default().fg(QENLO_THEME.accent) } else { Style::default().fg(QENLO_THEME.border) })
        .border_type(BorderType::Rounded)
    ...
    let filter_block = Block::default()
        .borders(Borders::ALL)
        .border_style(if app.search_active_field == 2 { Style::default().fg(QENLO_THEME.accent) } else { Style::default().fg(QENLO_THEME.border) })
        .border_type(BorderType::Rounded)
        .title(" 2. Filter User ID (optional) ")
        .style(Style::default().bg(QENLO_THEME.surface));
    let filter_p = Paragraph::new(if app.search_user_filter.is_empty() { "<No filter - all rows eligible>" } else { &app.search_user_filter })
        .block(filter_block)
        .style(if app.search_user_filter.is_empty() { Style::default().fg(QENLO_THEME.text_faint) } else { Style::default().fg(QENLO_THEME.text) });
    ...
    let k_gauge = Gauge::default()
        .block(k_block)
        .gauge_style(Style::default().fg(QENLO_THEME.accent).bg(QENLO_THEME.surface_raised))
    ...
    let action_text = vec![
        Line::from(vec![
            Span::styled(" [Enter] / [s] ", Style::default().fg(QENLO_THEME.accent).add_modifier(Modifier::BOLD)),
            Span::raw("Run Cosine Vector Query"),
        ]),
        Line::from(vec![
            Span::styled(" [r]           ", Style::default().fg(QENLO_THEME.accent).add_modifier(Modifier::BOLD)),
            Span::raw("Generate Random Query Vector"),
        ]),
        Line::from(vec![
            Span::styled(" [+/-]         ", Style::default().fg(QENLO_THEME.accent).add_modifier(Modifier::BOLD)),
            Span::raw("Adjust Result Limit k"),
        ]),
        Line::from(vec![
            Span::styled(" [Tab]         ", Style::default().fg(QENLO_THEME.accent).add_modifier(Modifier::BOLD)),
    ...
    let results_items: Vec<ListItem> = if app.search_results.is_empty() {
        ...
        app.search_results
            ...
            .map(|(idx, hit)| {
                ...
                let user_str = hit.record.as_ref().map_or("".to_string(), |r| format!(" · User: {}", r.user_id));
                let ts_str = hit.record.as_ref().map_or("".to_string(), |r| format!(" · Ts: {}", r.timestamp));

                ListItem::new(Line::from(vec![
                    Span::styled(format!(" #{:<2} ", idx + 1), Style::default().fg(QENLO_THEME.bg).bg(QENLO_THEME.accent).add_modifier(Modifier::BOLD)),
                    Span::raw(" "),
                    Span::styled(format!("ID #{:<6}", hit.id), Style::default().fg(QENLO_THEME.text).add_modifier(Modifier::BOLD)),
                    Span::styled(format!(" Dist: {:.4} ", hit.distance), Style::default().fg(QENLO_THEME.text_muted)),
                    Span::styled(format!("[{bar_str}] {:.3} sim", hit.similarity), Style::default().fg(QENLO_THEME.ok)),
                    Span::styled(format!("{user_str}{ts_str}"), Style::default().fg(QENLO_THEME.text_faint)),
                ]))
            })
            .collect()
    };
    ...
    let results_list = List::new(results_items)
        .block(
            Block::default()
                ...
                .border_style(Style::default().fg(QENLO_THEME.border))
                .border_type(BorderType::Rounded)
                .title(format!(" Top-{} Nearest Neighbors ", app.search_results.len()))
                .style(Style::default().bg(QENLO_THEME.surface)),
        );
...
fn render_storage_view(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let summary = if let Some(s) = &app.storage_details {
        format!(
            ...
            s.generation,
            s.durable_generation.map_or("None".to_string(), |g| g.to_string()),
    ...
    let sum_p = Paragraph::new(vec![
        Line::from(Span::styled(" Qenlo Storage & Compaction Manager", Style::default().fg(QENLO_THEME.accent).add_modifier(Modifier::BOLD))),
        Line::from(summary),
        Line::from(Span::styled(" Actions: [f] Flush WAL & Compact to Snapshot  │  [e] Export Snapshot  │  [r] Refresh", Style::default().fg(QENLO_THEME.text_muted))),
    ])
    .block(
        Block::default()
            .borders(Borders::ALL)
    ...
    let file_rows: Vec<Row> = if let Some(s) = &app.storage_details {
        s.files
            ...
            .map(|f| {
                Row::new(vec![
                    Line::from(Span::styled(&f.name, Style::default().fg(QENLO_THEME.text).add_modifier(Modifier::BOLD))),
                    Line::from(Span::styled(&f.kind, Style::default().fg(QENLO_THEME.ok))),
                    Line::from(format_bytes(f.size_bytes)),
                    Line::from(Span::styled(&f.path, Style::default().fg(QENLO_THEME.text_muted))),
                ])
            })
            .collect()
    } else {
    ...
    .header(
        Row::new(vec!["FILE NAME", "TYPE", "SIZE", "PATH"])
            .style(Style::default().bg(QENLO_THEME.surface_raised).fg(QENLO_THEME.text_muted))
            .bottom_margin(1),
    )
    .block(
        Block::default()
...
fn render_diagnostics_view(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let text = vec![
        Line::from(Span::styled(" Qenlo Native Engine & Diagnostics", Style::default().fg(QENLO_THEME.accent).add_modifier(Modifier::BOLD))),
        Line::raw(""),
        Line::from(format!(" • Host OS:                  {os}")),
        Line::from(format!(" • Target Architecture:      {arch}")),
        Line::from(vec![
            Span::raw(" • CPU SIMD Acceleration:    "),
            Span::styled(cpu_path, Style::default().fg(QENLO_THEME.ok).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(format!(" • Dimension:                {}D", app.status.dimension)),
...
fn render_help_view(frame: &mut Frame, _app: &App, area: Rect) {
    let text = vec![
        Line::from(Span::styled(" Keyboard Shortcuts & Navigation Reference", Style::default().fg(QENLO_THEME.accent).add_modifier(Modifier::BOLD))),
        Line::raw(""),
        Line::from(" [Tab] / [Shift+Tab]   Switch active navigation tab"),
        Line::from(" [1] - [4]             Jump directly to tab (1: Rows, 2: Search, 3: Storage, 4: Diag)"),
        ...
        Line::from(" [:]                   Claude Code-style command prompt (:open, :create, :flush, :quit)"),
        Line::from(" [q] / [Ctrl+C]        Quit QenloDB Browser"),
        Line::raw(""),
        Line::from(Span::styled(" Interactive Commands (press ':' to activate):", Style::default().fg(QENLO_THEME.accent))),
...
fn render_bottom_bar(frame: &mut Frame, app: &App, area: Rect) {
    ...
    if app.filter_mode {
        let filter_line = Line::from(vec![
            Span::styled(" Filter (user_id): ", Style::default().fg(QENLO_THEME.accent).add_modifier(Modifier::BOLD)),
    ...
    let status_span = if let Some((msg, _, is_err)) = &app.status_message {
        if *is_err {
            Span::styled(format!(" ✕ {msg}"), Style::default().fg(QENLO_THEME.bad).add_modifier(Modifier::BOLD))
        } else {
...
fn render_inspect_modal(frame: &mut Frame, record: &crate::state::RecordDto, area: Rect) {
    ...
    let status_str = if record.live { "Active" } else { "Tombstone (Deleted)" };
    ...
    let lines = vec![
        Line::from(vec![
            Span::styled(format!("Record ID: #{}", record.id), Style::default().fg(QENLO_THEME.accent).add_modifier(Modifier::BOLD)),
            Span::raw("   │   "),
            Span::styled(format!("User ID: {}", record.user_id), Style::default().fg(QENLO_THEME.text)),
            Span::raw("   │   "),
            Span::styled(format!("Timestamp: {}", record.timestamp), Style::default().fg(QENLO_THEME.text)),
            Span::raw("   │   "),
            Span::styled(format!("Status: {status_str}"), Style::default().fg(if record.live { QENLO_THEME.ok } else { QENLO_THEME.bad })),
        ]),
        Line::raw(""),
        Line::from(Span::styled(format!("Normalized Vector Components ({} dims):", record.vector.len()), Style::default().fg(QENLO_THEME.text_muted))),
        Line::from(Span::styled(format!("[{vec_formatted}]"), Style::default().fg(QENLO_THEME.text))),
        Line::raw(""),
        Line::from(Span::styled("Press [Esc] or [Enter] to close", Style::default().fg(QENLO_THEME.accent))),
    ];
...
fn render_add_modal(frame: &mut Frame, app: &App, area: Rect) {
    ...
    for (label, val, idx) in fields {
        ...
        let style = if is_active {
            Style::default().fg(QENLO_THEME.accent).add_modifier(Modifier::BOLD)
        } else {
            Style::default().fg(QENLO_THEME.text)
        };

        lines.push(Line::from(Span::styled(format!("{prefix}{label}"), style)));
        lines.push(Line::from(Span::styled(format!("    {val}{}", if is_active { "█" } else { "" }), Style::default().fg(QENLO_THEME.text))));
        lines.push(Line::raw(""));
    }
