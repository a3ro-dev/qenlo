...
fn main() {
    ...
    queue.write_buffer(
        &params,
        0,
        &[iters.to_le_bytes(), [0; 4], [0; 4], [0; 4]].concat(),
    );
    ...
    eprintln!(
        "heater cycles={cycles} mean_busy_ms={:.3} duty={:.3}",
        busy.as_secs_f64() * 1e3 / cycles as f64,
        busy.as_secs_f64() / start.elapsed().as_secs_f64()
    );
}
